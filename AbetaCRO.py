...
def larvaemutation_function(brooders, pos):
    ...
    brooders[range(nbrooders), pos] = np.logical_not(
        brooders[range(nbrooders), pos])
    return (brooders)
...
def larvaesettling(REEF, REEFpob, REEFfitness, larvae, larvaefitness, k):
    ...
    REEF, REEFpob, REEFfitness = _settle_larvae(
        larvae_emptycoral, fitness_emptycoral, REEF, REEFpob, REEFfitness, free)
...
def extremedepredation(REEF, REEFpob, REEFfitness, ke, empty_coral, empty_coral_fitness):
    (U, indices, count) = np.unique(
        REEFpob, return_index=True, return_counts=True, axis=0)
...
def coral_reef(dataset):
    ...
    trainX, testX, trainy, testy = train_test_split(
        data, label, stratify=label, test_size=test_size, random_state=(7+17*int(time.time() % 1000)))
    # clf=MLPClassifier(alpha=0.001, hidden_layer_sizes=(1000,500,100),max_iter=2000, random_state=4)
    clf = KNeighborsClassifier(n_neighbors=5)
    ...
    for n in range(Ngen):
        ...
        (REEF, REEFpob, REEFfitness) = larvaesettling(
            REEF, REEFpob, REEFfitness, larvae, larvaefitness, k)

        # Asexual reproduction
        (Alarvae, Afitness) = budding(REEF, REEFpob, REEFfitness)
        (REEF, REEFpob, REEFfitness) = larvaesettling(
            REEF, REEFpob, REEFfitness, Alarvae, Afitness, k)

        if n != Ngen:
            (REEF, REEFpob, REEFfitness) = depredation(
                REEF, REEFpob, REEFfitness, empty_coral, empty_coral_fitness)
            (REEF, REEFpob, REEFfitness) = extremedepredation(REEF, REEFpob,
                                                              REEFfitness, int(np.round(ke*N*M)), empty_coral, empty_coral_fitness)

        # Adaptive BETA
        for i in range(len(REEFpob)):
            REEFpob[i], REEFfitness[i] = adaptiveBeta(
                REEFpob[i], REEFfitness[i], trainX, testX, trainy, testy)
